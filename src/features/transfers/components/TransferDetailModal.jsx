        ...
        <div className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{transfer.fromAccount}</p>
            </div>
            <div>
              <p style={{ color: getColor('--color-text-secondary', '--color-dark-text-secondary') }} className="text-sm font-medium">
                Cuenta Destino
              </p>
              <p className="text-lg font-bold">{transfer.toAccount}</p>
            </div>
            <div>
              <p style={{ color: getColor('--color-text-secondary', '--color-dark-text-secondary') }} className="text-sm font-medium">
                Fecha
              </p>
              <p className="text-lg font-bold">{new Date(transfer.createdAt).toLocaleDateString()}</p>
            </div>
            <div className="col-span-1 md:col-span-2">
              <p style={{ color: getColor('--color-text-secondary', '--color-dark-text-secondary') }} className="text-sm font-medium">
                Concepto
              </p>
              <p className="text-sm">{transfer.concept}</p>
            </div>
          </div>
        </div>
